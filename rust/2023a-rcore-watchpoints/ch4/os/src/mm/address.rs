...
impl From<usize> for PhysPageNum {
    ...
    } //input usize  ouput uszie
}
// 因此，如果输入 0x10000000（在十进制中是 268435456），
// 它的二进制表示形式大于 39 位，
// 所以最终结果将是 0x00000000，即只有低 39 位的值保留在 VirtAddr 类型中
...
impl VirtPageNum {
    ...
    pub fn indexes(&self) -> [usize; 3] {
        let mut vpn = self.0; // 从 VirtPageNum 中获取虚拟页号，0是第一个成员结构
        let mut idx = [0usize; 3]; // 用于存储 页表项 索引的数组，初始值都为0
        for i in (0..3).rev() { // 对于虚拟页号的每个级别（通常有三个级别的页表）
            idx[i] = vpn & 511;
            vpn >>= 9;
        }
        idx
