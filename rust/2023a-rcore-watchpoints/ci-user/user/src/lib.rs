...
// 静态数组用于初始化用户堆
//https://blog.csdn.net/qq_42779423/article/details/107568766
...
fn clear_bss() {
    ...
    unsafe {
        ...
        .fill(0);
    }// 函数创建了一个可变的 u8 指针数组，代表未初始化的全局变量区域。
}


//，mut[u8] 表示一个可变的字节数组或切片，你可以修改其中的字节数据。在你的代码中，core::slice::from_raw_parts_mut 返回的就是这样一个可变的字节切片，用于处理未初始化的全局变量区域。
// 内核入口点，初始化内核，处理命令行参数并调用用户程序的main函数
...
// 启用弱链接特性
//https://www.cnblogs.com/fyqs/p/17683377.html
#[linkage = "weak"]
#[no_mangle]
