...
pub fn syscall(id: usize, args: [usize; 3]) -> isize {
    ...
    unsafe {
        core::arch::asm!(
            "ecall",                        // 使用 "ecall" 指令执行系统调用
            inlateout("x10") args[0] => ret, // 将系统调用号传递给 "x10" 寄存器，将结果存储在 "ret" 中
            in("x11") args[1],              // 将参数1传递给 "x11" 寄存器
            in("x12") args[2],              // 将参数2传递给 "x12" 寄存器
            in("x17") id                    // 将系统调用号传递给 "x17" 寄存器
        );
    }
    ret // 返回系统调用结果
}
